...
def define_generator(latent_dim=(50,), signal_shape=(200, 67), label_shape=(2,)):
    ...
    # gen=200,32 x li=200,2 x si=200,67 ## Uncomment this
...
def define_gan(g_model, d_model, latent_dim=(200, 67), signal_shape=(200, 67), label_shape=(2,)):
    ...
    model = Model([g_model.input[0], g_model.input[1],
                  g_model.input[2]], [out1, out2])
    ...
    model.compile(loss=['mse', 'categorical_crossentropy'],
                  optimizer=opt, loss_weights=[1, 10], metrics=["accuracy"])
    model.summary()
    return model
